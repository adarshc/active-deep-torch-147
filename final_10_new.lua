...
local label     = require 'subset_classes'
-- print(label)
...
opt.save = opt.save .. '_' .. experiment_code .. '_c' .. total_classes .. '_spc' .. samples_per_class .. '_bs' .. opt.batchSize .. '_lr' .. opt.learningRate .. '_lrd' .. opt.learningRateDecay .. '_wd' .. opt.weightDecay .. '_m' .. opt.momentum .. '_' .. os.date("%y%m%d%H%M%S")
print(opt)
---------------------------------------------------------------------------------------------------------
...
elseif network == 'big' then
   ...
   net:add(nn.ReLU(true))
   net:add(SpatialConvolutionMM(4096, 2048, 1, 1, 1, 1)) -- added layer
   net:add(nn.ReLU(true))
   net:add(SpatialConvolutionMM(2048, 1024, 1, 1, 1, 1)) -- added layer
   net:add(nn.ReLU(true))
   net:add(SpatialConvolutionMM(1024, total_classes, 1, 1, 1, 1)) -- added layer
   net:add(nn.View(total_classes))
   -- net:add(nn.ReLU(true))
   -- net:add(SpatialConvolutionMM(1000, total_classes, 1, 1, 1, 1))
   -- net:add(nn.View(total_classes))
...
local train_files = require 'subset_train_images'
local test_files = require 'subset_test_images'
local train_labels = require 'subset_train_labels'
local test_labels = require 'subset_test_labels'
...
-- initialising training set as 2 images per class
...
-----------------------------------------------------------------------------------------
function calculateNewDeeperEntropy(sample)
  local input = sample
  if tensorType == 'double' then input = input:double()
  elseif tensorType == 'cuda' then input = input:cuda() end

  local committee_1 = net:forward(input)
  local temp_tensor_1 = committee_1:clone()
  --measure_1 = 0
  --for i = 1,total_classes do
  --  measure_1 = measure_1 + (-1.0)*(committee_1[i])*math.exp(committee_1[i])
  --end
  temp_tensor_1:apply(math.exp)
  temp_tensor_1:cmul(committee_1)
  temp_tensor_1:neg()
  measure_1 = temp_tensor_1:sum()

  local committee_2 = net:get(21).output
  committee_2:add(1.0)
  --local shaping_vector_21 = torch.Tensor(committee_2:nElement())
  --committee_2 = committee_2:resizeAs(shaping_vector_21:cuda())
  --measure_2 = 0
  --temp_sum_2 = 0
  --for i = 1,committee_2:nElement() do
  --  temp_sum_2 = temp_sum_2 + committee_2[i]
  --end
  --for i = 1,committee_2:nElement() do
  --  committee_2[i] = committee_2[i]/temp_sum_2
  --end
  --for i = 1,committee_2:nElement() do
  --  if committee_2[i] ~= 0 then
  --    measure_2 = measure_2 + (-1.0)*(committee_2[i])*math.log(committee_2[i])
  --  end
  --end
  temp_sum_2 = committee_2:sum()
  committee_2:div(temp_sum_2)
  local temp_tensor_2 = committee_2:clone()
  temp_tensor_2:log()
  temp_tensor_2:neg()
  temp_tensor_2:cmul(committee_2)
  measure_2 = temp_tensor_2:sum()

  local committee_3 = net:get(23).output
  committee_3:add(1.0)
  --local shaping_vector_23 = torch.Tensor(committee_3:nElement())
  --committee_3 = committee_3:resizeAs(shaping_vector_23:cuda())
  --measure_3 = 0
  --temp_sum_3 = 0
  --for i = 1,committee_3:nElement() do
  --  temp_sum_3 = temp_sum_3 + committee_3[i]
  --end
  --for i = 1,committee_3:nElement() do
  --  committee_3[i] = committee_3[i]/temp_sum_3
  --end
  --for i = 1,committee_3:nElement() do
  --  if committee_3[i] ~= 0 then
  --    measure_3 = measure_3 + (-1.0)*(committee_3[i])*math.log(committee_3[i])
  --  end
  --end
  temp_sum_3 = committee_3:sum()
  committee_3:div(temp_sum_3)
  local temp_tensor_3 = committee_3:clone()
  temp_tensor_3:log()
  temp_tensor_3:neg()
  temp_tensor_3:cmul(committee_3)
  measure_3 = temp_tensor_3:sum()
  -- print(committee_2)
  -- print(committee_3)
  --print(temp_sum_2)
  --print(temp_sum_3)
...
-----------------------------------------------------------------------------------------
function calculateDeeperEntropy(sample)
  local input = sample
  if tensorType == 'double' then input = input:double()
  elseif tensorType == 'cuda' then input = input:cuda() end
  ...
  temp_tensor_2:neg()
  temp_tensor_2:cmul(committee_2)
  temp_tensor_2[temp_tensor_2:ne(temp_tensor_2)] = 0
  measure_2 = temp_tensor_2:sum()
  --temp_tensor_2:view(temp_tensor_2:nElement())
  --temp_tensor_2:reshape(temp_tensor_2:nElement())
  --measure_2 = 0
  --for i = 1,temp_tensor_2:nElement() do
  --  measure_2 = measure_2 + temp_tensor_2[i]
  --end
  ...
  temp_tensor_3:neg()
  temp_tensor_3:cmul(committee_3)
  temp_tensor_3[temp_tensor_3:ne(temp_tensor_3)] = 0
  measure_3 = temp_tensor_3:sum()
  --temp_tensor_3:view(temp_tensor_3:nElement())
  --temp_tensor_3:reshape(temp_tensor_3:nElement())
  --measure_3 = 0
  --for i = 1,temp_tensor_3:nElement() do
  --  measure_3 = measure_3 + temp_tensor_3[i]
  --end
...
function train()
   ...
   local correct_predictions = 0
    ...
    --         local current_measure = calculateDeeperEntropy(trainData.data[j])
    ...
    if active_learning_flag then
      ...
      for i = 1,trsize do
        ...
        if untrained_training_images[i] then
          entropy_measures[i] = calculateDeeperEntropy(trainData.data[i])
        else
...
--   (19): nn.ReLU
--   (20): nn.SpatialConvolutionMM(4096 -> 2048, 1x1)
--   (21): nn.ReLU
--   (22): nn.SpatialConvolutionMM(2048 -> 1024, 1x1)
--   (23): nn.ReLU
--   (24): nn.SpatialConvolutionMM(1024 -> 20, 1x1)
--   (25): nn.View(20)
--   (26): nn.LogSoftMax
-- }
print '==> the main loop'
...
--temp = net:get(20).output
--temp_result = torch.Tensor(temp:nElement())
--temp = temp:resizeAs(temp_result:cuda())
--print(pred)
--print(temp)
--sum = 0
--for i = 1,total_classes do
...
--end
--new_sum = pred:apply(math.exp):sum()
--print('==> sum of predicted values for each class: ' .. new_sum)
--for i = 1,100 do
--  current_measure = calculateDeeperEntropy(trainData.data[i])
--  print('==> deeper entropy measure: ' .. current_measure)
--end
--for i = 1,100 do
--  xlua.progress(i, 100)
--  current_measure = calculateDeeperEntropy(trainData.data[i])
