...
local label     = require 'fs_subset_classes'
-- print(label)
...
local train_files = require 'fs_subset_train_images'
local test_files = require 'fs_subset_test_images'
local train_labels = require 'fs_subset_train_labels'
local test_labels = require 'fs_subset_test_labels'
...
function calculateDensityWeightedUncertainty()
  ...
  centroids, counts = unsup.kmeans(points, total_classes, k_means_iterations, total_classes, nil, true)
...
function train()
   ...
   local correct_predictions = 0
    ...
    if active_learning_flag then
      ...
      density_weighted_uncertainty_measures, unlabeled_training_indices = calculateDensityWeightedUncertainty()

      top_k_values, top_k_indices = density_weighted_uncertainty_measures:topk(samples_added_per_update)
...
--print('[Clustering unlabeled samples...]')
--local points = torch.Tensor((trsize - current_pool_size), 4096)
--local counter = 0
--for i = 1,trsize do
--  if untrained_training_images[i]  then
...
--    --my_output = net:get(19).output
--    --points[counter] = my_output
--    points[i] = calculateSimilarity(trainData.data[i])
--  end
--end
--local points = torch.Tensor(calculateSimilarity())
--local my_table = {}
--my_table = calculateSimilarity()
--points = torch.Tensor(my_table)
--print((#points)[1])
--print((#points)[2])
--local centroids, counts = unsup.kmeans(points, total_classes, 40, total_classes, nil, true)
--print(counts)
--local centroids, counts = calculateSimilarity()
--print((#centroids)[1])
--print((#centroids)[2])
--print(counts:div(40))
--print(counts:sum())
--sample_result, sample_indices = calculateDensityWeightedUncertainty()
--print(sample_result)
--for i = 1,100 do
