...
function calculateEntropy(sample)
  ...
  --  result = result + (-1.0)*(prediction[i])*math.exp(prediction[i])
  --end
  local probability_distribution = prediction:clone()
  probability_distribution:apply(math.exp)
  probability_distribution:cmul(prediction)
  probability_distribution:neg()
  result = probability_distribution:sum()
  return result
end
-----------------------------------------------------------------------------------------
    ...
    if active_learning_flag then
      ...
      for i = 1,trsize do
        ...
        if untrained_training_images[i] then
          entropy_measures[i] = calculateEntropy(trainData.data[i])
        else
...
--for i = 1,100 do
--  current_measure = calculateEntropy(trainData.data[i])
--  print('==> current entropy measure: ' .. current_measure)
--end
-----------------------------------------------------------------------------------------------------------------------
