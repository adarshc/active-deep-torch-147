...
elseif network == 'big' then
   ...
   net:add(nn.ReLU(true))
   net:add(SpatialConvolutionMM(4096, total_classes, 1, 1, 1, 1)) -- added layer
   net:add(nn.View(total_classes))
   -- net:add(nn.ReLU(true))
   -- net:add(SpatialConvolutionMM(1000, total_classes, 1, 1, 1, 1))
   -- net:add(nn.View(total_classes))
...
-----------------------------------------------------------------------------------------
function calculateNewDeepEntropy(sample)
  local input = sample
  ...
  local committee_2 = net:get(17).output
  committee_2:add(1.0)
  --local shaping_vector_17 = torch.Tensor(committee_2:nElement())
  --committee_2 = committee_2:resizeAs(shaping_vector_17:cuda())
  --measure_2 = 0
  --temp_sum_2 = 0
  ...
  local committee_3 = net:get(19).output
  committee_3:add(1.0)
  --local shaping_vector_19 = torch.Tensor(committee_3:nElement())
  --committee_3 = committee_3:resizeAs(shaping_vector_19:cuda())
  --measure_3 = 0
  --temp_sum_3 = 0
...
-----------------------------------------------------------------------------------------
function calculateDeepEntropy(sample)
  local input = sample
  if tensorType == 'double' then input = input:double()
  elseif tensorType == 'cuda' then input = input:cuda() end
...
function train()
   ...
   local correct_predictions = 0
    ...
    --         local current_measure = calculateDeepEntropy(trainData.data[j])
    ...
    if active_learning_flag then
      ...
      for i = 1,trsize do
        ...
        if untrained_training_images[i] then
          entropy_measures[i] = calculateDeepEntropy(trainData.data[i])
        else
...
--for i = 1,100 do
--  current_measure = calculateDeepEntropy(trainData.data[i])
--  print('==> deep entropy measure: ' .. current_measure)
--end
--for i = 1,100 do
--  xlua.progress(i, 100)
--  current_measure = calculateDeepEntropy(trainData.data[i])
