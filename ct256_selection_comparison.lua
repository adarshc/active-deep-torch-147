-- ct256_selection_comparison.lua
------------------------------------------------------------------------------------------------------
require 'torch'   -- torch
...
local label     = require 'ct256_subset_classes'
-- print(label)
...
train_path = '/home/cse/adarshc/Thesis/Caltech-256/'
test_path = '/home/cse/adarshc/Thesis/Caltech-256/'

local train_files = require 'ct256_subset_train_images'
local test_files = require 'ct256_subset_test_images'
local train_labels = require 'ct256_subset_train_labels'
local test_labels = require 'ct256_subset_test_labels'
...
-----------------------------------------------------------------------------------------
function calculateDensityWeightedUncertainty()
  result = torch.Tensor((trsize - current_pool_size))
  unlabeled_training_indices = torch.Tensor((trsize - current_pool_size))

  print('[Computing feature vectors and uncertainties of unlabeled pool samples...]')
  local points = torch.Tensor((trsize - current_pool_size), 4096)
  local counter = 0, input, prediction, current_output
  for i = 1,trsize do
    xlua.progress(i, trsize)
    if untrained_training_images[i]  then
      counter = counter + 1
      ...
      points[counter] = current_output:clone():float()
      unlabeled_training_indices[counter] = i

      local probability_distribution = prediction:clone()
      probability_distribution:apply(math.exp)
      result[counter] = torch.max(probability_distribution)
    end
  end
  ...
  for i = 1,(trsize - current_pool_size) do
    xlua.progress(i, (trsize - current_pool_size))
    local aggregate_similarity_measure = 0, average_similarity_measure
    for j = 1,total_classes do
  ...
  result:cmul(information_density_measures)
...
function train()
   ...
   local correct_predictions = 0
    ...
    if active_learning_flag then
      ...
      --  end
      --end
      confidence_measures = torch.Tensor((trsize - current_pool_size))
      unlabeled_training_indices = torch.Tensor((trsize - current_pool_size))
      lc_counter = 0
      for i = 1,trsize do
        xlua.progress(i, trsize)
        if untrained_training_images[i] then
          ...
          confidence_measures[lc_counter] = calculateConfidence(trainData.data[i])
          unlabeled_training_indices[lc_counter] = i
        end
      end
...
while initial_training_counter > 0 do
  train()
  initial_training_counter = initial_training_counter - 1
end
...
print('==> applying least confidence measure...')
local lc_samples = {}
local lc_labels = {}
local lc_measures = torch.Tensor((trsize - current_pool_size))
unlabeled_training_indices = torch.Tensor((trsize - current_pool_size))
lc_counter = 0
for i = 1,trsize do
  xlua.progress(i, trsize)
  if untrained_training_images[i] then
    lc_counter = lc_counter + 1
    lc_measures[lc_counter] = calculateConfidence(trainData.data[i])
    unlabeled_training_indices[lc_counter] = i
  end
end
samples_added_per_update = math.min(samples_added_per_update,(training_threshold - current_pool_size))
top_k_values, top_k_indices = lc_measures:topk(samples_added_per_update, true, true)
for i = 1,samples_added_per_update do
  table.insert(lc_samples, trainData.data[unlabeled_training_indices[top_k_indices[i]]])
  table.insert(lc_labels, trainData.labels[unlabeled_training_indices[top_k_indices[i]]])
end
local directory_name = paths.concat(opt.save, 'lc_actively_selected_samples')
os.execute('mkdir -p ' .. directory_name)
for counter = 1,samples_added_per_update do
  image.save(paths.concat(directory_name, 'sample_' .. counter .. '_' .. label[lc_labels[counter]] .. '.jpg'), lc_samples[counter])
end

print('==> applying expected gradient length measure...')
local egl_samples = {}
local egl_labels = {}
local egl_measures = torch.Tensor((trsize - current_pool_size))
unlabeled_training_indices = torch.Tensor((trsize - current_pool_size))
egl_counter = 0
for i = 1,trsize do
  xlua.progress(i, trsize)
  if untrained_training_images[i] then
    egl_counter = egl_counter + 1
    egl_measures[egl_counter] = calculateExpectedGradientLength(trainData.data[i])
    unlabeled_training_indices[egl_counter] = i
  end
end
samples_added_per_update = math.min(samples_added_per_update,(training_threshold - current_pool_size))
top_k_values, top_k_indices = egl_measures:topk(samples_added_per_update, true, true)
for i = 1,samples_added_per_update do
  table.insert(egl_samples, trainData.data[unlabeled_training_indices[top_k_indices[i]]])
  table.insert(egl_labels, trainData.labels[unlabeled_training_indices[top_k_indices[i]]])
end
local directory_name = paths.concat(opt.save, 'egl_actively_selected_samples')
os.execute('mkdir -p ' .. directory_name)
for counter = 1,samples_added_per_update do
  image.save(paths.concat(directory_name, 'sample_' .. counter .. '_' .. label[egl_labels[counter]] .. '.jpg'), egl_samples[counter])
end

print('==> applying kernel farthest first measure...')
local kff_samples = {}
local kff_labels = {}
local kff_measures, unlabeled_training_indices = calculateKernelDistance()
samples_added_per_update = math.min(samples_added_per_update,(training_threshold - current_pool_size))
top_k_values, top_k_indices = kff_measures:topk(samples_added_per_update, true, true)
for i = 1,samples_added_per_update do
  table.insert(kff_samples, trainData.data[unlabeled_training_indices[top_k_indices[i]]])
  table.insert(kff_labels, trainData.labels[unlabeled_training_indices[top_k_indices[i]]])
end
local directory_name = paths.concat(opt.save, 'kff_actively_selected_samples')
os.execute('mkdir -p ' .. directory_name)
for counter = 1,samples_added_per_update do
  image.save(paths.concat(directory_name, 'sample_' .. counter .. '_' .. label[kff_labels[counter]] .. '.jpg'), kff_samples[counter])
end

print('==> applying density weighted least confidence measure...')
local dwlc_samples = {}
local dwlc_labels = {}
local dwlc_measures, unlabeled_training_indices = calculateDensityWeightedUncertainty()
samples_added_per_update = math.min(samples_added_per_update,(training_threshold - current_pool_size))
top_k_values, top_k_indices = dwlc_measures:topk(samples_added_per_update, false, true)
for i = 1,samples_added_per_update do
  table.insert(dwlc_samples, trainData.data[unlabeled_training_indices[top_k_indices[i]]])
  table.insert(dwlc_labels, trainData.labels[unlabeled_training_indices[top_k_indices[i]]])
end
local directory_name = paths.concat(opt.save, 'dwlc_actively_selected_samples')
os.execute('mkdir -p ' .. directory_name)
for counter = 1,samples_added_per_update do
  image.save(paths.concat(directory_name, 'sample_' .. counter .. '_' .. label[dwlc_labels[counter]] .. '.jpg'), dwlc_samples[counter])
end

print('==> applying density weighted expected gradient length measure...')
local dwegl_samples = {}
local dwegl_labels = {}
local dwegl_measures, unlabeled_training_indices = calculateDensityWeightedExpectedModelChange()
samples_added_per_update = math.min(samples_added_per_update,(training_threshold - current_pool_size))
top_k_values, top_k_indices = dwegl_measures:topk(samples_added_per_update, true, true)
for i = 1,samples_added_per_update do
  table.insert(dwegl_samples, trainData.data[unlabeled_training_indices[top_k_indices[i]]])
  table.insert(dwegl_labels, trainData.labels[unlabeled_training_indices[top_k_indices[i]]])
end
local directory_name = paths.concat(opt.save, 'dwegl_actively_selected_samples')
os.execute('mkdir -p ' .. directory_name)
for counter = 1,samples_added_per_update do
  image.save(paths.concat(directory_name, 'sample_' .. counter .. '_' .. label[dwegl_labels[counter]] .. '.jpg'), dwegl_samples[counter])
end
-----------------------------------------------------------------------------------------------------------------------
